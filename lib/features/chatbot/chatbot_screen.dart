...
class Classifier {
  static const Map<String, Map<String, List<String>>> _keywords = {
    'adult_choking': {
      'en': [
        'choking', 'choke', 'stuck', 'throat', 'cannot breathe',
        "can't breathe", 'turning blue', 'heimlich', 'airway blocked',
        'object in throat', 'food stuck',
      ],
      'ar': [
        'اختناق', 'يختنق', 'عالق', 'حلق', 'لا يتنفس',
        'لا يستطيع التنفس', 'ازرق', 'انسداد', 'شيء في الحلق',
        'طعام عالق', 'مسدود',
      ],
    },
    'child_choking': {
      'en': [
        'baby choking', 'infant choking', 'child choking',
        'toddler choking', 'kid choking', 'swallowed',
        'baby blue', 'infant not breathing',
      ],
      'ar': [
        'طفل يختنق', 'رضيع يختنق', 'ابتلع', 'طفل صغير يختنق',
        'طفل عالق', 'رضيع لا يتنفس', 'طفل ازرق',
      ],
    },
    'asthma': {
      'en': [
        'asthma', 'inhaler', 'wheezing', 'breathing difficulty',
        'short of breath', 'tight chest', "can't breathe properly",
        'chest tight', 'wheeze', 'no inhaler',
      ],
      'ar': [
        'ربو', 'بخاخ', 'صفير', 'ضيق تنفس', 'صعوبة التنفس',
        'صدر ضيق', 'لا يتنفس بشكل صحيح', 'أزمة ربو', 'نوبة ربو',
      ],
    },
    'anaphylaxis': {
      'en': [
        'allergy', 'allergic reaction', 'anaphylaxis', 'epipen',
        'swollen throat', 'hives', 'bee sting', 'food allergy',
        'swollen face', 'rash all over', 'severe reaction',
      ],
      'ar': [
        'حساسية', 'رد فعل تحسسي', 'صدمة تأقية', 'تورم الحلق',
        'طفح', 'لسعة نحلة', 'حساسية طعام', 'تورم الوجه',
        'طفح جلدي', 'حساسية شديدة',
      ],
    },
    'unconscious_breathing': {
      'en': [
        'unconscious', 'unresponsive', 'passed out', 'fainted',
        'not waking', 'breathing but unconscious', 'collapsed',
        'won\'t wake up', 'eyes rolled',
      ],
      'ar': [
        'فاقد الوعي', 'لا يستجيب', 'أغمي عليه', 'إغماء',
        'لا يصحى', 'يتنفس لكن فاقد الوعي', 'انهار', 'سقط مغشياً',
      ],
    },
    'not_breathing_cpr': {
      'en': [
        'not breathing', 'no pulse', 'heart stopped', 'cardiac arrest',
        'cpr', 'resuscitation', 'no heartbeat', 'heart attack',
        'no breathing at all', 'stopped breathing',
      ],
      'ar': [
        'لا يتنفس', 'لا نبض', 'قلب توقف', 'سكتة قلبية',
        'إنعاش', 'انعاش قلبي', 'لا دقات قلب', 'أزمة قلبية',
        'نوبة قلبية', 'توقف التنفس',
      ],
    },
    'bleeding': {
      'en': [
        'bleeding', 'blood', 'cut', 'wound', 'hemorrhage',
        'won\'t stop bleeding', 'deep cut', 'gash', 'stabbed',
        'laceration',
      ],
      'ar': [
        'نزيف', 'دم', 'جرح', 'قطع', 'لا يتوقف النزيف',
        'جرح عميق', 'طعنة', 'دم كثير',
      ],
    },
    'burns': {
      'en': [
        'burn', 'burned', 'fire', 'scalded', 'hot water burn',
        'chemical burn', 'skin burned', 'blister',
      ],
      'ar': [
        'حرق', 'محروق', 'نار', 'ماء ساخن', 'حرق كيميائي',
        'بثور', 'جلد محروق',
      ],
    },
    'fracture': {
      'en': [
        'broken bone', 'fracture', 'broken arm', 'broken leg',
        'broken wrist', 'bone sticking out', 'cannot move arm',
        'snapped', 'deformed limb',
      ],
      'ar': [
        'كسر', 'عظمة مكسورة', 'ذراع مكسور', 'ساق مكسورة',
        'لا يستطيع تحريك يده', 'عظمة بارزة',
      ],
    },
    'seizure': {
      'en': [
        'seizure', 'convulsion', 'epilepsy', 'shaking uncontrollably',
        'fits', 'twitching', 'epileptic', 'falling shaking',
      ],
      'ar': [
        'نوبة', 'تشنج', 'صرع', 'اهتزاز لا إرادي', 'نوبة صرع',
        'تشنجات', 'يرتجف',
      ],
    },
    'stroke': {
      'en': [
        ...
        'sudden confusion', 'fast stroke', 'brain attack',
        'one side weak', 'facial drooping',
      ],
      'ar': [
        'سكتة دماغية', 'وجه مائل', 'ضعف ذراع', 'كلام غير واضح',
        'ارتباك مفاجئ', 'جلطة دماغية', 'جانب ضعيف',
      ],
    },
  };
  ...
  static const Map<String, List<String>> _childMarkers = {
    'en': [
      'baby', 'infant', 'child', 'toddler', 'kid', 'boy',
      'girl', 'son', 'daughter', 'years old', 'months old',
    ],
    'ar': [
      'طفل', 'رضيع', 'بيبي', 'ولد', 'بنت', 'ابن',
      'ابنة', 'سنوات', 'شهر', 'طفلة',
    ],
  };
  ...
  static Map<String, dynamic> predict(String text, {String? forcedLang}) {
    ...
    // boost child_choking if child markers detected
    if ((hits['adult_choking'] ?? 0) > 0 || (hits['child_choking'] ?? 0) > 0) {
    ...
    if (totalHits == 0) {
      return {'category': 'unknown', 'confidence': 0.0, 'urgency': 'unknown', 'lang': lang};
    }
    ...
    if (confidence < 0.30) {
      return {'category': 'unknown', 'confidence': confidence, 'urgency': 'unknown', 'lang': lang};
    }
  ...
  static String _urgencyFor(String cat) {
    const high = ['not_breathing_cpr', 'adult_choking', 'child_choking', 'anaphylaxis', 'stroke', 'seizure'];
    const med  = ['asthma', 'unconscious_breathing', 'bleeding', 'burns'];
